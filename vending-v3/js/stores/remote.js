...
layui.use(['table', 'form', 'layer','tree'], function () {
    ...
    $('.footerBtn .RdetermineBtn').click(function () {
        ...
        loadingAjax('/api/app/uploadApp', 'post', addAppJson, sessionStorage.token, 'mask', 'addAppCOnt', 'addAppBox', layer).then(res => {
            console.log(res)
    ...
    $('.editAppCont .rdetermineBtn').click(function(){
        ...
        loadingAjax('/api/app/updateAppInfo','post',editAppJson,sessionStorage.token,'mask','editAppCont','editAppBox',layer).then(res=>{
            layer.msg(res.message,{icon:1});
            tableIns.reload({
                where: {}
            });
    ...
    // loadingAjax('/api/updateApp','post',JSON.stringify({machine:'2824d5365328780c'})).then(res=>{
