import {
    loadAjax, loadingWith, toastTitle, showPopup,
    closeParents, closeWindow, timeStamp, treeList, numFormat2, getKeyTime, timeFlag
} from '../../../common/common';
...
$('.validationContent .confirmBtn').click(function () {
    ...
    loadAjax('/user/verifyAlonePwd', 'post', sessionStorage.token, IPassWord, '', '.validationContent', 'top').then(res => {
        if (orderData.payType == 0) {
            ...
            loadAjax('/pay/refund_icbc', 'post', sessionStorage.token,
                refundData, 'mask', '.refundCont', 'top30').then(res => {
                toastTitle(res.message, 'success');
                orderArrList(merchantIdStr, NumderIndex);
                $('.goodsCont').hide();
            }).catch(err => {
            ...
            loadAjax('/pay/sandRefund', 'post', sessionStorage.token,
                refundData, 'mask', '.refundCont', 'top30').then(res => {
                toastTitle(res.message, 'success');
                orderArrList(merchantIdStr, NumderIndex);
                $('.goodsCont').hide();
            ...
            loadAjax('/pay/refund_digitalCurrency', 'post', sessionStorage.token,
                refundData, 'mask', '.refundCont', 'top30').then(res => {
                toastTitle(res.message, 'success');
                orderArrList(merchantIdStr, NumderIndex);
                $('.goodsCont').hide();
