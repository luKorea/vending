...
layui.use(['table', 'form', 'layer', 'tree'], function () {
    ...
    $('.ListOperation .edit').click(function () {
        ...
        if (payData.payName == '微信') {
            ...
            $('.changePay .numberPay').hide();
            $('.changePay .Alipay').hide();
        } else if (payData.payType == 6) {
            $('.changePay .numberPay').show();
            $('.changePay .IcbcPay').hide();
            $('.changePay .juhePay').hide();
            $('.changePay .WeChat').hide();
            $('.changePay .Alipay').hide();
        ...
        form.val("SetPay", {
            ...
            app_key_number: tyoe === '6' ? payData.app_key : '',
            numberId: tyoe === '6' ? payData.mchId : '',
            app_id_number: tyoe === '6' ? payData.app_id : '',
            alipay_public_key_number: tyoe === '6' ? payData.alipay_public_key : '',
            app_private_key_number: tyoe === '6' ? payData.app_private_key : '',
        });
    ...
    $('.changeBody .submit_btn').click(function () {
        ...
        if (type === '2') {
            ...
            var textParam = JSON.stringify({
                alipay_public_key: payFormData.alipay_public_key_number,
                mchId: payFormData.numberId,
                app_id: payFormData.app_id_number,
        ...
        loadingAjax(editUrl, 'post', textParam, sessionStorage.token, 'mask', '', '', layer).then(res => {
            var editPay = JSON.stringify({
                ...
                payee: payFormData.payee,
                payType: type,
                app_id: type === '2' ? payFormData.officialId :
                    type === '4' || type === '5' ? payFormData.juheapp_id :
                        type === '6' ? payFormData.app_id_number :
                            payFormData.aliPayId,
                app_key: type === '2' ? payFormData.app_key :
                    type === '4' || type === '5' ? payFormData.juheapp_key :
                        type === '6' ? payFormData.app_key_number :
                            '',
                mchId: type === '2' ? payFormData.MerchantsId :
                    (type === '4' || type === '5') ? payFormData.juhemchId :
                        type === '6' ? payFormData.numberId : '',
                alipay_public_key: type === '1' ? payFormData.alipay_public_key :
                    type === '4' || type === '5' ? juheUploadEditGongVal :
                        type === '6' ? payData.alipay_public_key_number :
                            '',
                app_private_key: type === '1' ? payFormData.app_private_key :
                    type === '4' || type === '5' ? juheUploadEditSiVal :
                        type === '6' ? payFormData.app_private_key_number :
                            payFormData.MerchantsKey,
            ...
            loadingAjax('/pay/updatePayParam', 'post', editPay, sessionStorage.token, 'mask', 'changePay', 'changeBox', layer).then(res => {
                ...
                form.val("SetPay", {
                    ...
                    alipay_public_key: '',
                    app_private_key: '',
                    alipay_public_key_number: '',
                    app_private_key_number: '',
                    MerchantsKey: '',
                })
            }).catch(err => {
                layer.msg(err.message, {icon: 2})
    ...
    form.on('select(addSelect)', function (data) {
        ...
        if (typeSelect === 1) {
            ...
            $('.addePay .Alipay').hide();
            $('.addePay .juhePay').hide();
        } else if (typeSelect === 4 || typeSelect === 5) {
    ...
    $('.addePay .submit_btn').click(function () {
        ...
        $('.mask').fadeIn();
        $('.maskSpan').addClass('maskIcon');
        if (typeIndex !== '2' && typeIndex !== '1' && typeIndex !== '3' && typeIndex !== '4' && typeIndex !== '5' && typeIndex !== '6') {
            var editPay = JSON.stringify({
                merchantId: merchantsPay,
                payee: addData.payee,
        ...
        // 杉德公钥证书(.cer)   alipay_public_key
            ...
            var textParam = JSON.stringify({
                alipay_public_key: addData.alipay_public_key_number,
                mchId: addData.numberId,
                app_id: addData.app_id_number,
        ...
        loadingAjax(editUrl, 'post', textParam, sessionStorage.token, 'mask', '', '', layer).then(res => {
            var editPay = JSON.stringify({
                ...
                payee: addData.payee,
                payType: typeIndex,
                app_id: typeIndex === '2' ? addData.officialId :
                ...
                alipay_public_key: typeIndex === '1' ? addData.alipay_public_key :
                    (typeIndex === '4' || typeIndex === '5') ? juheUploadGongVal :
                        typeIndex === '6' ? addData.alipay_public_key_number :
                            '',
                app_private_key: typeIndex === '1' ? addData.app_private_key :
                    (typeIndex === '4' || typeIndex === '5') ? juheUploadSiVal :
                        typeIndex === '6' ? addData.app_private_key_number :
                            addData.MerchantsKey
            })
