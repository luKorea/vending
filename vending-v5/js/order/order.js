...
layui.use(['laydate', 'table', 'tree', 'flow', 'layer', 'form'], function () {
                ...
                // }},
                {
                    field: 'shipStatus', width: 150, title: '出货状态', align: 'center', templet: function (d) {
                        if (d.shipStatus || d.shipStatus == 0) {
                            var shipArr = ['未出货', '部分出货失败', '全部出货成功', '出货中', '全部出货失败'];
                            return shipArr[d.shipStatus]
                        } else {
                            return '-'
                        }
    ...
    table.on('row(moneyData)', function (obj) {
        ...
        $('.orderInformation button span').html(
            (obj.data.shipStatus === 0 ? '未出货' :
                    obj.data.shipStatus === 1 ? '部分出货失败' :
                        obj.data.shipStatus === 2 ? '全部出货成功' :
                            obj.data.shipStatus === 3 ? '出货中' : '全部出货失败'
            ));
