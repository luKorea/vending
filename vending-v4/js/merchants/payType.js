...
// 添加
  ...
  $('.addpayType .RdetermineBtn').click(function(){
    ...
    loadingAjax('/pay/newPayType','post',AddType,sessionStorage.token,'','addpayType','addBox',layer).then(res=>{
  ...
  table.on('tool(payTypeList)', function (obj) {
    ...
    if (obj.event == 'edit') {
        ...
        layer.confirm('确定删除？', function (index) {
            ...
            loadingAjax('/pay/deletePayType','post',TypeId,sessionStorage.token,'','','',layer).then(res=>{
        ...
        layer.confirm(payTypeData.status==1?'确定禁用？':'确定启用？', function (index) {
         ...
         loadingAjax('/pay/updatePayType','post',editType,sessionStorage.token,'','editpayType','editBox',layer).then(res=>{
                console.log(res)
                layer.msg(res.message,{icon:1})
                tableIns.reload({
                    where:{}
