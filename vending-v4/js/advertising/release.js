...
layui.use(['element', 'laydate', 'table', 'carousel', 'tree', 'form'], function () {
    ...
    function machineDetailsFun(id) {
        machineList1 = table.render({
            elem: '#machine1',
            url: `${vApi}/publicized/getPublicizedMachine`,
            method: 'post',
            contentType: "application/json",
            headers: {
                token,
    ...
    $('.mandatroFooter div').click(function () {
      var  PType = $(this).attr('Ptype');
        ...
        loadingAjax('/publicized/sendMerchantAd', 'post', RData, sessionStorage.token, 'mask', 'chooseLower', 'chooseBox', layer).then((res) => {
    ...
    function functionReleaseFun() {
        releaseHistoryList = table.render({
            ...
            url: `${vApi}/publicized/getAdHistory`,
            height: 500,
            method: 'post',
            contentType: "application/json",
            headers: {
    ...
    $('.pushReleaseListBtn').click(function () {
        ...
        if (receiveList.data.length > 0) {
            ...
            loadingAjax('/publicized/getMerchantAd', 'post', numberArr, sessionStorage.token, 'mask', 'topGoodsList', 'topBox', layer).then((res) => {
                layer.msg(res.message, { icon: 1 });
                advertisingLis.reload({
                    where: {}
                });
